...
def addToDB():
  ...
  #now = datetime.date('06','03','1990')
  yearNow = request.form['year']
  monthNow = request.form['month']
  dayNow = request.form['day']
  #now = datetime.date(request.form['month'], request.form['day'], request.form['year'])
  newDate_Collected = '('+'yearNow-'+'monthNow-'+'dayNow'+')'
  # newDate_Collected = request.form['month, day, year']
  ...
  insert = "INSERT INTO seed_stock (stock_id, cross_id, genotype, generation, female_parent, male_parent, species, date_collected, location, contributor_id, antibiotics_resistance, oligo_1, oligo_2, notes) VALUES( %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s ,%s, %s)"
        # print insert
  try:
    cur.execute(insert, (newStock_id, newCross_id, newGenotype, newGeneration, newFemale_Parent, newMale_Parent, newSpecies, newDate_Collected, newLocation, newContributor_id, newAntibiotics, newOligo1, newOligo2, newNotes))
    #cur.execute(insert)
    conn.commit()
    # except:
    #   print "Error"
